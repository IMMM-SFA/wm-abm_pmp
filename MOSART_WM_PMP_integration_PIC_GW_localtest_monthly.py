...
if year_int<1950:  # If year is before 1950 (warm-up period), use the external baseline water demand files
    ...
    if year == '1940':
        hist_avail_bias = pd.read_csv('C:\\Users\\yoon644\\OneDrive - PNNL\\Documents\\PyProjects\\wm_netcdf\\hist_avail_bias_correction_20230112.csv')
        aggregation_functions={'WRM_SUPPLY_acreftmth_OG': 'sum','sw_avail_bias_corr': 'sum', 'RIVER_DISCHARGE_OVER_LAND_LIQ_OG': 'mean'}
        hist_avail_bias = hist_avail_bias.groupby(['NLDAS_ID'], as_index=False, dropna=False).aggregate(aggregation_functions)
...
for month in months:
    ...
    # new_fname = '/pic/projects/im3/wm/Jim/pmp_input_files/demand_input/test_'+ str_year + '_' + month + '.nc' # define ABM demand input directory
