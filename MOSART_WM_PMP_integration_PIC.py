...
logging.basicConfig(filename='/pic/scratch/yoon644/csmruns/jimtest2/run/app.log',level=logging.INFO)
...
def calc_demand(year, month):
    ...
    try:
        logging.info('Trying to run ABM calc for month: ' + month)
        if int(month) == 1:
            ...
            if year_int==2000:
                ...
                pic_input_dir = '/pic/scratch/yoon644/csmruns/jimtest2/run/'

                # loop through .nc files and extract data
                first = True
                for m in months:
                    #dataset_name = 'jim_abm_integration.mosart.h0.' + str(year-1) + '-' + m + '.nc'
                    dataset_name = 'jimtest2.mosart.h0.' + str(year_int - 1) + '-' + m + '.nc'
            ...
            data_file=pd.ExcelFile("/pic/projects/im3/wm/Jim/pmp_input_files/MOSART_WM_PMP_inputs_v1.xlsx")
            ...
            with open('/pic/projects/im3/wm/Jim/pmp_input_files/land_constraints_by_farm.p', 'rb') as fp:
                land_constraints_by_farm = pickle.load(fp)

            # Load gammas and alphas
            with open('/pic/projects/im3/wm/Jim/pmp_input_files/gammas.p', 'rb') as fp:
                gammas = pickle.load(fp)
            with open('/pic/projects/im3/wm/Jim/pmp_input_files/net_prices.p', 'rb') as fp:
