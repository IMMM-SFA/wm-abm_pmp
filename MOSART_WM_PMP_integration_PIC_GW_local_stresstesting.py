...
# JY to run crop price changes
crop_price_mult = 1
for key in net_prices_total:
    net_prices_total[key] = (crop_price_mult * data_profit.iloc[key]['price'] * data_profit.iloc[key]['yield']) - data_profit.iloc[key]['land_cost'] - alphas_total[key]
...
for n in range(int(no_of_chunks)):
    ...
    for key in sw_constraints_by_farm_subset:
        sw_constraints_by_farm_subset[key] *= 0.5
...
results_pivot = pd.pivot_table(results_pd, index=['nldas'], values=['calc_gw_demand', 'calc_sw_demand', 'calc_total_demand','xs_gw','xs_sw','xs_total'], aggfunc=np.sum)  #JY demand is order of magnitude low, double check calcs
results_pivot = results_pivot.reset_index()
logging.info('Compiled results from all chunks and calculated demand')

results_pivot.to_csv('abm_stress_gwminus50perc.csv')
...
results_pivot_baseline['perc_diff'] = (results_pivot_baseline['xs_total'] - results_pivot_baseline['xs_total_baseline']) / results_pivot_baseline['xs_total_baseline']
results_pivot_baseline = results_pivot_baseline.fillna(0)

import matplotlib.pyplot as plt
plt.scatter(x=results_pivot_baseline['xs_total_baseline'], y=results_pivot_baseline['xs_total'], s=2)
plt.xlabel('Baseline')
plt.ylabel('Stress Test')
limit = max(plt.xlim()[1], plt.ylim()[1])
plt.xlim(0.0, limit)
plt.ylim(0.0, limit)
plt.xlim(0.0, 4950.672189002475)
plt.ylim(0.0, 4950.672189002475)
plt.show()



import matplotlib.pyplot as plt
plt.scatter(x=results_pivot_baseline['calc_gw_demand_baseline'], y=results_pivot_baseline['calc_gw_demand'], s=2)
plt.xlabel('Baseline')
plt.ylabel('Stress Test')
limit = max(plt.xlim()[1], plt.ylim()[1])
plt.xlim(0.0, limit)
plt.ylim(0.0, limit)
# plt.xlim(0.0, 0.1483543024151486)
# plt.ylim(0.0, 0.1483543024151486)
plt.show()

import matplotlib.pyplot as plt
plt.scatter(x=results_pivot_baseline['calc_sw_demand_baseline'], y=results_pivot_baseline['calc_sw_demand'], s=2)
plt.xlabel('Baseline')
plt.ylabel('Stress Test')
limit = max(plt.xlim()[1], plt.ylim()[1])
plt.xlim(0.0, limit)
plt.ylim(0.0, limit)
# plt.xlim(0.0, 0.04953731369682469)
# plt.ylim(0.0, 0.04953731369682469)
plt.show()
...
area_irrigated_farm = pd.merge(area_irrigated_farm, results_pivot_baseline[['nldas','xs_total']], on='nldas')

import matplotlib.pyplot as plt
plt.scatter(x=area_irrigated_farm['area_irrigated'], y=results_pivot_baseline['xs_total'], s=2)
plt.xlabel('Observed')
plt.ylabel('Modeled (Baseline)')
limit = max(plt.xlim()[1], plt.ylim()[1])
plt.xlim(0.0, limit)
plt.ylim(0.0, limit)
# plt.xlim(0.0, 0.1483543024151486)
