...
os.chdir('C:\\Users\\yoon644\\OneDrive - PNNL\\Documents\\IM3\\Paper #1\\Nature Communications submission\\Revision\\results\\20220418 abm')

for year in range(15):
    print(str(year))
    for m in months:
        year_str = str(year+1950)
        ds = xr.open_dataset('wm_abm_run.mosart.h0.' + year_str + '-' + m + '.nc')
        df = ds.to_dataframe()
    ...
    if year == 0:
        ...
        df_summary['year'] = year+1950
    else:
        aggregation_functions = {'WRM_SUPPLY': 'mean','WRM_DEMAND0': 'mean','RIVER_DISCHARGE_OVER_LAND_LIQ': 'mean','WRM_STORAGE': 'mean','NAME': 'first', 'State': 'first', 'ERS_region': 'first', 'COUNTYFP': 'first'}
        df_summary_toappend = df_append.groupby(['NLDAS_ID'], as_index=False).aggregate(aggregation_functions)
        df_summary_toappend['year'] = year+1950
        df_summary = df_summary.append(df_summary_toappend)

df_summary[['year','NLDAS_ID','WRM_SUPPLY','WRM_DEMAND0','RIVER_DISCHARGE_OVER_LAND_LIQ','WRM_STORAGE']].to_csv('wm_summary_results_20220418_abm.csv')
#
